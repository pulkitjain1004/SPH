...
print(i)
print(j)
p1temp = matrix(0,27,max(m.new,n.new)+10)   #R will NOT automaticall add columns if required
#toString(p1temp)
rownames(p1temp) = c('a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z','_')

p2temp = matrix(0,27,max(m.new,n.new)+10)   #R will NOT automaticall add columns if required
#toString(p2temp)
...
# print('Traceback direction from last character')
  ...
  p1temp = p1temp[,1:k-1]       #Dirty way to get rid of OUT OF BOUNDS
  p2temp = p2temp[,1:k-1]
